...
class RollCreditsEffect : public OneTimeEffect
{
    ...
    static inline float  currentPosition;
...
public:
    ...
    Hooked_CCredits_Render ()
    {
        ...
        lineOffset = 0;
        scrollOffset = float(CTimer::m_snTimeInMilliseconds - CCredits::CreditsStartTime) / 23.0f; // magic number
        
        CFont::SetBackground(false, false);
        CFont::SetCentreSize(RsGlobal.maximumHeight * 0.92f);
        ...
        const auto PrintCreditText_ = [&](const char* textKey, float scale, bool highlighted = false) {
            auto text = reinterpret_cast<unsigned short*>((char*)textKey);
            CCredits::PrintCreditText(scale, scale, text, lineOffset, scrollOffset, highlighted);
        };

        PrintCreditText_("SA-CHAOS 1.0", SCALE_BIG, true);
        for(int i = 0; i < 40; i++)
            PrintCreditText_("Test", SCALE_SMALL);
    ...
    Real_CCredits_Render ()
    {
        ...
        const auto PrintCreditText_ = [&](const char* textKey, float scale, bool highlighted = false) {
            auto text = reinterpret_cast<unsigned short*>(TheText.Get(textKey));
            CCredits::PrintCreditText(scale, scale, text, lineOffset, scrollOffset, highlighted);
        };

        PrintCreditText_("CRED000", SCALE_BIG, true);
        CCredits::PrintCreditSpace(SPACE_BIG, lineOffset);
        ...
        CCredits::PrintCreditSpace(SPACE_NORMAL, lineOffset);
        ...
        CCredits::PrintCreditSpace(SPACE_SMALL, lineOffset);
